...
static const size_t primes_table_[] = {
        0,
        1,
        3,
        5,
...
static void rehash_(ing_hash_table* ht, size_t new_bucket_count) {
    ...
    for(size_t i = 0; i < old_array.size; ++i) {
        ...
        while(node) {
            ...
            node_copy->next = ing_dynamic_array_at_as_void_ptr(&ht->array_of_linked_list, index);
...
static void conditional_rehash_(ing_hash_table* ht, size_t additional_entries_count) {
    ...
    if(current_load_factor > max_load_factor) {
        while(primes_table_[++ht->index_in_size_table] < (ing_hash_table_buckets_count(ht) + additional_entries_count)) {}
    }
}
